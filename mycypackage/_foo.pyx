...
cdef extern from "../cpp_code/foo.hpp":
    ...
    void multiply_array_via_span(int * v, int len, int factor)
...
def double_span_cpp_nocopy(arr: np.ndarray[np.int32_t]):
    # First, we check if the array is contiguous. If not, we make it contiguous
    if not arr.flags['C_CONTIGUOUS']:
        arr = np.ascontiguousarray(arr, dtype=np.int32)
    ...
    # This function modifies the original array
    multiply_array_via_span(<int*> &arr_view[0], arr_view.shape[0], 2)    